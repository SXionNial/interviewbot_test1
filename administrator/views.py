...
from user.models import Account, CreateJob, AppliedJob, Questions, FileSubmission, WorkExperience, Skills, Education

from django.template.loader import render_to_string
from django.shortcuts import render, redirect
from django.views.generic import View, CreateView, FormView, DeleteView
    ...
    def post(self, request):
        if request.method == 'POST':
            ...
            if checkbox != 'defaultQ':
                ...
                form_joboffering = CreateJob(
                    title=jobTitle, description=jobDescription, admin_id=request.user.id)
...
class JobListsView(View):
    ...
    def post(self, request):
        ...
        if 'btnDelete' in request.POST:
            ...
            # if is_default == "1":
            if is_default == "1" or is_default == "1":
                ...
                CreateJob.objects.filter(id=job_id).update(
            ...
            if checkbox != 'defaultQ':
                ...
                form_joboffering = CreateJob(
                    title=jobTitle, description=jobDescription, admin_id=user.id)
                form_joboffering.save()
                form_questions = Questions(
                    job_id=form_joboffering.id, is_default=0, question_1=q1, question_2=q2, question_3=q3, question_4=q4, question_5=q5, question_6=q6, question_7=q7, question_8=q8,
                    question_9=q9, question_10=q10, question_11=q11, question_12=q12, question_13=q13, question_14=q14, question_15=q15, question_16=q16,
...
class StaffRemoveView(View):
    def get(self, request):
        users = Account.objects.filter(staff=1).values()
        context = {
            'users': users,
        }
        return render(request, 'remove_staff.html', context)

    def post(self, request):
        if request.method == "POST":
            if 'btnDelete' in request.POST:
                email = request.POST.get("email_id")
                Account.objects.filter(id=email).delete()
...
class ApplicantForfeitedView(View):
    def get(self, request):
        if not request.user.staff:
            return redirect('user:access_denied_view')

        forfeited = AppliedJob.objects.select_related('user').filter(final_score=0) | AppliedJob.objects.select_related('user').filter(final_score__isnull=True)

        context = {
            'forfeited': forfeited,
        }

        return render(request, 'job_applicants_forfeited.html', context)

class ViewFileSubmission(View):
    def get(self, request):
        if not request.user.staff:
            return redirect('user:access_denied_view')
        ...
        return render(request, 'file_submission_admin.html', context)