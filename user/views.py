    ...
    try:
        if request.session['instruction'] and not request.session['q1']:
            AppliedJob.objects.filter(
                job_id=request.session['job'], user_id=request.user.id).delete()
        elif not request.session['instruction']:
            AppliedJob.objects.filter(
                job_id=request.session['job'], user_id=request.user.id).update(final_score=0)
    ...
    def post(self, request):
        if request.method == 'POST':
            if 'btnCancel' in request.POST:
                ...
                return redirect('user:file_submission_view')
...
class JobInterviewQ1View(View):
    ...
    def post(self, request):
        try:
            ...
            final_score = finalScoring(positive, job_weight, float(
                minutes), float(seconds), job_timer)
...
class JobInterviewQ20View(View):
    ...
    def post(self, request):
        try:
            ...
            # Calculate the final score
            score20 = finalScoring(positive, job_weight20, float(
                minutes), float(seconds), job_timer)

            # get final score for the overall interview session
            user_appliedJob = AppliedJob.objects.filter(
...
class FileSubmissionView(View):
    def get(self, request):
        try:
            ...
            if request.session['instruction'] == True:
                interview_job = request.session['job']
                job = CreateJob.objects.filter(id=interview_job)
                context = {
                    'job': job
                ...
                return redirect('user:interview_forfeit_view', context)
        except KeyError:
            return redirect('user:interview_forfeit_view')
    ...
    def post(self, request):
        ...
        if request.method == "POST":
            if 'btnProceed' in request.POST:
                # loop array for inputsss
                # for workkk
                arr_title = request.POST.getlist('title[]')
                arr_companyAdd = request.POST.getlist('companyAdd[]')
                arr_year = request.POST.getlist('work_year[]')
                arr_achievements = request.POST.getlist('work_achievements[]')
                
                for index, x in enumerate(arr_title):
                    workExperience = WorkExperience()
                    workExperience.user_id = request.user.id
                    workExperience.job_id = request.session['job']
                    workExperience.title = arr_title[index]
                    workExperience.companyAdd = arr_companyAdd[index]
                    workExperience.work_year = arr_year[index]
                    workExperience.work_achievements = arr_achievements[index]
                    workExperience.save()
                
                # for educcc
                arr_education= request.POST.getlist('education[]')
                arr_eyear = request.POST.getlist('educ_year[]')
                arr_course = request.POST.getlist('course[]')
                arr_eachievements = request.POST.getlist('educ_achievements[]')
                
                for index, x in enumerate(arr_education):
                    educationn = Education()
                    educationn.user_id = request.user.id
                    educationn.job_id = request.session['job']
                    educationn.education = arr_education[index]
                    educationn.educ_year = arr_eyear[index]
                    educationn.course_taken = arr_course[index]
                    educationn.educ_achievements = arr_eachievements[index]
                    educationn.save()

                # for skillers
                arr_skill = request.POST.getlist('skills[]')
                
                for index, x in enumerate(arr_skill):
                    skill = Skills()
                    skill.user_id = request.user.id
                    skill.job_id = request.session['job']
                    skill.skills = arr_skill[index]
                  
                    skill.save()

                file = FileSubmission()
                file.user_id = request.user.id
                file.job_id = request.session['job']

                if len(request.FILES) != 0:
                    file.file_upload = request.FILES['file_upload']

                file.phone = request.POST['phone']
                file.email = request.POST['email']
                file.address = request.POST['address']
                file.job_objective = request.POST['job_objective']
                # prod.work_experience = request.POST['work_experience']
                # file.education = request.POST['education']
                # file.skills = request.POST['skills']
                file.bdate = request.POST['bdate']
                file.save()
